...
sw_skin_sep <- starwars %>% # start from starwars data
  separate(col = skin_color, # separate the `skin color` column
           into = c("skin_1","skin_2"), # into two columns
           sep = ",", # Creating the separation at the first *comma*
           extra = "merge", # If additional colors, merge all into second column
           remove = FALSE) # And *do not* remove the original column
```
...
mpg_model_class <- mpg %>% # start with mpg
  unite(col = model_class, # create a new column called 'model_class'
        c(model, class), # that combines the model & class columns
...
some_dates <- c(01012000, 04161997, 10121962) #creates a vector of dates w/o spaces
some_dates_converted <- mdy(some_dates) #converts to Date, stores as some_dates_converted
...
gasol_hookshots <- lakers %>% 
  filter(player == "Pau Gasol") %>% 
  filter(type == "hook") %>% 
  mutate(date_new = ymd(date)) %>% 
  mutate(year_new = year(date_new)) %>% 
  mutate(month_new = month(date_new))

shots_per_month <- gasol_hookshots %>% 
  count(month_new)
```
...
storms_danger <- storms %>% #start with storms data
  mutate(danger = case_when( #add a new column danger, that contains...
    status == "hurricane" ~ "high", # "danger" if storm status = hurricane
    status %in% c("tropical storm", "tropical depression") ~ "moderate" # "moderate" for others
  ))
```
